# Codira Package Registry Service Specification
...
- [4. Endpoints](#4-endpoints)
  ...
  - [4.3. Fetch manifest for a package release](#43-fetch-manifest-for-a-package-release)
    - [4.3.1. codira-version query parameter](#431-codira-version-query-parameter)
...
- _Package_:
  A named collection of Codira source code
  that is organized into one or more modules
  according to a `Package.code` manifest file.
- _Scope_:
...
Accept: application/vnd.code.registry.v1+json
```
...
| [\[3\]](#endpoint-3) | `GET`  | `/{scope}/{name}/{version}/Package.code{?codira-version}` | Fetch manifest for a package release              |
...
`application/vnd.code.registry.v1+json`
...
and redirect to the unqualified `Package.code` resource.

```http
HTTP/1.1 303 See Other
Content-Version: 1
Location: https://packages.example.com/mona/LinkedList/1.1.1/Package.code
```
...
X-Codira-Package-Signature-Format: cms-1.0.0
X-Codira-Package-Signature: l1TdTeIuGdNsO1FQ0ptD64F5nSSOsQ5WzhM6/7KsHRuLHfTsggnyIWr0DxMcBj5F40zfplwntXAgS0ynlqvlFw==
```
...
Host: packages.example.com
Accept: application/vnd.code.registry.v1
```
...
A client MUST set a `X-Codira-Package-Signature-Format` header 
...
{
   "detail": "package doesn't contain a valid manifest (Package.code) file"
}
```

A server SHOULD use the `codira package compute-checksum` tool
...
[CocoaPods]: https://cocoapods.org "A dependency manager for Codira and Objective-C Cocoa projects"
