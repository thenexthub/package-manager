...
This guide provides a brief overview of Codira Package Manager plugins, describes how a package can make use of plugins, and shows how to get started writing your own plugins.

## Overview

Some of Codira Package Manager's functionality can be extended through _plugins_.  Package plugins are written in Codira using the `PackagePlugin` API provided by the Codira Package Manager.  This is similar to how the Codira Package manifest itself is implemented as a Codira script that runs as needed in order to produce the information CodiraPM needs.

A plugin is represented in the CodiraPM package manifest as a target of the `pluginTarget` type — and if it should be available to other packages, there also needs to be a corresponding `pluginProduct` target.  Source code for a plugin is normally located in a directory under the `Plugins` directory in the package, but this can be customized.
...
Plugins have access to a representation of the package model, and plugins that define custom commands can also invoke services provided by CodiraPM to build and test products and targets defined in the package to which the plugin is applied.
...
This will cause CodiraPM to call the plugin, passing it a simplified version of the package model for the target to which it is being applied.  Any build commands returned by the plugin will be incorporated into the build graph and will run at the appropriate time during the build.
...
The Codira script files that implement the logic of the plugin are expected to be in a directory named the same as the plugin, located under the `Plugins` subdirectory of the package.  This can be overridden with a `path` parameter in the `pluginTarget`.
...
By default, Codira Package Manager looks for the implementation of a declared plugin in a subdirectory of the `Plugins` directory named with the same name as the plugin target.  This can be overridden using the `path` parameter in the target declaration.

A plugin consists of one or more Codira source files, and the main entry point of the build tool plugin script is expected to conform to the `BuildToolPlugin` protocol.
...
The plugin script can import *Foundation* and other standard libraries, but in the current version of CodiraPM, it cannot import other libraries.
...
In the current version of Codira Package Manager, plugins can only use standard system libraries (and not those from other packages, such as CodiraArgumentParser).  Consequently, this plugin uses the built-in `ArgumentExtractor` helper in the *PackagePlugin* module to do simple argument extraction.
