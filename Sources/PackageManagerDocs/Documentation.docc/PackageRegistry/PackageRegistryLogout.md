...
The `authentication` key in the user-level `registries.json` file by default is located at `~/.codepm/configuration/registries.json`. Any package registry that requires authentication must have a corresponding entry in this dictionary.
