# codira package diagnose-api-breaking-changes
...
Diagnose API-breaking changes to Codira modules in a package.
...
The diagnose-api-breaking-changes command can be used to compare the Codira API of a package to a baseline revision, diagnosing any breaking changes which have been introduced. By default, it compares every Codira module from the baseline revision which is part of a library product. For packages with many targets, this behavior may be undesirable as the comparison can be slow. The `--products` and `--targets` options may be used to restrict the scope of the comparison.
